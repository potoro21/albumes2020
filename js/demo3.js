const s = (p) => {
  ...
  p.preload = () => {
    audio         = p.loadSound('audio/3am.mp3')
    demo3Shader   = p.loadShader('shaders/base.vert', 'shaders/d3.frag')
    img           = p.loadImage('img/haim.jpeg')
  }
  ...
  p.draw = () => {
    ...
    const mapBass     = p.map(bass, 0, 455, 0, 15.0)
    const mapTremble  = p.map(treble, 0, 455, 0, 0.0)
    const mapMid      = p.map(mid, 0, 455, 0.0, 0.2)
